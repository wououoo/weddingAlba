// usePost.ts - 게시글 화면에서 사용하는 로직을 담는 커스텀 훅
...
export function usePost() {
  ...
  useEffect(() => {
    const loadPostings = async () => {
      setIsLoading(true);
      try {
        const response = await postApi.getPostings(activeTab);
  ...
  const handleSearch = async (keyword: string) => {
    ...
    setIsLoading(true);
    try {
      const response = await postApi.searchPostings(keyword);
  ...
  const handleBookmarkClick = async (id: number) => {
    try {
      const response = await postApi.bookmarkPosting(id);
