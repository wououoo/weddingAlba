...
/**
 * 모집자(호스트) 리뷰를 관리하는 엔티티
 * 
 * 신청자가 모집자(호스트)에 대해 작성한 리뷰 정보를 관리합니다.
 * 신청자가 작성한 리뷰 내용, 점수, 작성 일시 등의 정보를 포함합니다.
 * 호스트에 대한 평판을 관리하고 신용도 시스템에 활용됩니다.
 */
...
public class HostReview {
    ...
    private Long hostReviewId;            // 호스트 리뷰 고유 식별자
    ...
    @Column(name = "user_id")
    private Long userId;                  // 호스트(모집자) 사용자 ID (리뷰 대상)
